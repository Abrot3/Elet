...
window.initScene3D = function () {
    ...
    scene3d.renderer.setPixelRatio(Math.min(window.devicePixelRatio, isMobile ? 1.2 : 1.5));
    scene3d.renderer.shadowMap.enabled = true;
    scene3d.renderer.shadowMap.type = isMobile ? THREE.BasicShadowMap : THREE.PCFSoftShadowMap;
...
window.initAutopsy3D = function () {
    ...
    autopsy3d.renderer.setPixelRatio(Math.min(window.devicePixelRatio, isMobile ? 1.2 : 1.5));
    autopsy3d.renderer.shadowMap.enabled = true;
    autopsy3d.renderer.shadowMap.type = isMobile ? THREE.BasicShadowMap : THREE.PCFSoftShadowMap;
    ...
    // Event Listeners
    autopsy3d.renderer.domElement.addEventListener("touchstart", onAutopsyTouch, { passive: true });
    autopsy3d.renderer.domElement.addEventListener("mousedown", onAutopsyClick);
...
function buildFallbackBody() {
    ...
    bodyGroup.add(legL);
    const legR = legL.clone();    bodyGroup.traverse(c => { if (c.isMesh) { c.castShadow = true; c.receiveShadow = true; } });
...
function onAutopsyTouch(e) {
    if (e.touches.length > 0) {
        const rect = autopsy3d.renderer.domElement.getBoundingClientRect();
        autopsy3d.mouse.x = ((e.touches[0].clientX - rect.left) / rect.width) * 2 - 1;
        autopsy3d.mouse.y = -((e.touches[0].clientY - rect.top) / rect.height) * 2 + 1;
        handleAutopsyRaycast();
    }
}

function onAutopsyClick(e) {
    const rect = autopsy3d.renderer.domElement.getBoundingClientRect();
...
window.disposeAutopsy3D = function () {
    ...
    if (autopsy3d.renderer) {
        autopsy3d.renderer.domElement.removeEventListener("touchstart", onAutopsyTouch);
        autopsy3d.renderer.domElement.removeEventListener("mousedown", onAutopsyClick);
